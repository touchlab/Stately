...
actual class AtomicReference<V> actual constructor(initialValue: V) {
    private val atom = AtomicReference(initialValue.maybeFreeze())
    actual fun get(): V = atom.value

    actual fun set(value_: V) {
        atom.value = value_.maybeFreeze()
    }
     ...
     */
    actual fun compareAndSet(expected: V, new: V): Boolean = atom.compareAndSet(expected, new.maybeFreeze())
}
