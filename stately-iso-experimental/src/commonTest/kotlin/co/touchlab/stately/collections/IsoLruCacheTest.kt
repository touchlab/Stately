...
class IsoLruCacheTest {
    data class MapData(val s: String)

    @Test
    fun testInitFrozen() {
        assertTrue(IsoLruCache<String, MapData>(4).isNativeFrozen)
    }

    @Test
    fun sanityCheck() {
        val collect = IsoMutableList<MapData>()
        val sc = IsoLruCache<String, MapData>(4) {
    ...
    fun onRemoveCanFreeze() {
        ...
        val sc = IsoLruCache<String, MapData>(2) {
            collect.add(it.value)
        }
    ...
    fun stress() {
        ...
        val sc = IsoLruCache<String, MapData>(MAX_CACHE_SIZE).freeze()
        ...
        worker3.runBackground {
            var count = 150_000_000
            while (currentTimeMillis() < stopTime) {
                sc.put("key $count", MapData("val $count"))
                count++
            }
    ...
    fun mtPutStress() {
        ...
        val ops = ThreadOperations<IsoLruCache<String, MapData>> {
            IsoLruCache(CACHE_SIZE) { count.incrementAndGet() }
        }
    ...
    private fun addEntries(count: Int, cache: IsoLruCache<String, MapData>, startAt: Int = 0) {
    ...
    private fun checkResults(c: List<*>, vararg elems: Any) {
