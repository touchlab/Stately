...
class SharedLruCacheTest {
  /*@Test
  fun testInitFrozen() {
    assertTrue(FastNativeLruCache<String, MapData>(4).isNativeFrozen())
  }

  @Test
  fun sanityCheck() {
    val collect = FastNativeLinkedList<MapData>()
    val sc = FastNativeLruCache<String, MapData>(4) {
  ...
  fun entries() {
    val collect = FastNativeLinkedList<MapData>()
    val sc = FastNativeLruCache<String, MapData>(4) {
      println("en dump before")
      collect.add(it.value)
      println("en dump after ${it.value}")
    }
  ...
  *//**
  ...
  fun onRemoveCanFreeze() {
    ...
    val sc = FastNativeLruCache<String, MapData>(2) {
      collect.add(it.value)
    }
  ...
  *//*@Test
  ...
  fun stress() {
    ...
    val sc = FastNativeLruCache<String, MapData>(MAX_CACHE_SIZE)
  ...
  fun newstressnew() {
    val MAX_CACHE_SIZE = 4
    val sc = FastNativeLruCache<String, MapData>(MAX_CACHE_SIZE).freeze()
    ...
    val stopTime = currentTimeMillis() + (15 * 60 * 1000)
    ...
    val futures = workers.mapIndexed { index, worker ->
      worker.execute(TransferMode.SAFE, {Triple(index, sc, stopTime)}) {
        val workCount = it.first
        val sc = it.second
        val stopTime = it.third
        var count = workCount * 50_000
        while (currentTimeMillis() < stopTime) {
          sc.put("key $count", MapData("val $count"))
          count++
          if(count % 10_000 == 0){
            println("worker $workCount count $count")
          }
        }
      }
    }
  ...
  fun mtPutStress() {
    ...
    val ops = ThreadOperations<FastNativeLruCache<String, MapData>> {
      FastNativeLruCache(CACHE_SIZE) { count.incrementAndGet() }
    }
  ...
  private fun addEntries(count: Int, cache: FastNativeLruCache<String, MapData>, startAt: Int = 0) {
