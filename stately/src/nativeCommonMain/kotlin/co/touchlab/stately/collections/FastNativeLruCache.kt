 ...
 */
class FastNativeLruCache<K, V>(
    private val maxCacheSize: Int,
    private val onRemove: (MutableMap.MutableEntry<K, V>) -> Unit = {}
) : LruCache<K, V> {

    private var lock: Lock = Lock()
    private val cacheMap = FastNativeHashMap<K, CacheEntry<K, V>>()
    private val cacheList = FastNativeLinkedList<K>()

    init {
        freeze()
    }

    internal fun debugPrint(){
        println("lru [${cacheList.debugPrint()}]")
    }
    /**
     * Stores value at key.
     *
    ...
    override fun put(key: K, value: V): V? {
        ...
        withLock {
            ...
            val node: FastNativeLinkedList<K>.Node
            val result: V?
            if (cacheEntry != null) {
    ...
    internal data class CacheEntry<K, V>(val v: V, val node: FastNativeLinkedList<K>.Node)
